...
exports.ProjectsController = {
    listAll: async (req, res) => {
        ...
        const findP = await prisma_1.prisma.project.findMany({
        ...
        const totalP = await prisma_1.prisma.project.count({
            where: JSON.parse(filterData.filter)
        });
        if (findP) {
    ...
    create: async (req, res) => {
        ...
        const createP = await prisma_1.prisma.project.create({
            data
        });
        if (createP) {
    ...
    delete: async (req, res) => {
        ...
        const findP = await prisma_1.prisma.project.findFirst({
            where: {
                id: parseInt(`${id}`) | 0
            }
        });
        ...
        const deleteP = await prisma_1.prisma.project.delete({
    ...
    update: async (req, res) => {
        ...
        const updateP = await prisma_1.prisma.project.updateMany({
            where: { id: parseInt(id) },
            data
        });
        if (updateP) {
